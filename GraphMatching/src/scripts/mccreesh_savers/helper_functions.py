...
def get_mappings(graph_pairs, exp_name, path):
    ...
    for file_name, directory_name in graph_pairs:
        mapping, algorithm = None, None
        if directory_name == None:
            print('unsolved ground truth pair')
        else:
            metadata_file_name = join(path, 'experiments', 'gpgnode-results', exp_name, directory_name, '{}.out'.format(file_name))
            ...
            while line:
                ...
                if (directory_name == 'james-cpp-max' and line[0:8] == 'Solution') \
                    ...
                    mapping = metadata_file.readline()[:-1]
                    algorithm = directory_name
                    metadata_file.close()
                    break
                line = metadata_file.readline()
            if mapping is None or algorithm is None:
...
def get_processed_mapping(mapping):
    ...
    return mapping_list, has_true_matching
